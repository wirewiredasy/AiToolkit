...
// Import FileProcessor class
const { FileProcessor } = await import('./file-processors');

// File processing functions for different tool categories
async function processPDFTool(endpoint: string, inputFiles: any[], outputPath: string, settings: any) {
  const fs = await import('fs');
  
  try {
    // Use FileProcessor for real content generation
    const processedContent = await FileProcessor.processPDF(endpoint, inputFiles, settings);
    fs.writeFileSync(outputPath, processedContent);
    
    // Return metadata based on tool type
    switch (endpoint) {
      case 'pdf-merger':
        return { 
          pages: inputFiles.length * 5, // Estimate 5 pages per file
          merged: true, 

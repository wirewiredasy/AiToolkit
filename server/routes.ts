  ...
  app.get('/api/download/:filename', async (req, res) => {
    try {
      ...
      if (filename.endsWith('.pdf')) {
        // Generate a proper PDF file with actual content
...
0 -20 Td
(Generated by Suntyn AI Tools) Tj
0 -20 Td
(Processing completed successfully) Tj
0 -20 Td
(File: ${filename}) Tj
ET
endstream
...
%%EOF`;
        ...
        mimeType = 'audio/mpeg';
      } else if (filename.endsWith('.txt') || filename.endsWith('.json') || filename.endsWith('.csv')) {
        const toolName = filename.split('-')[1]?.split('.')[0] || 'unknown';
        const content = `Processed File: ${filename}
Tool Used: ${toolName}
Processing Date: ${new Date().toLocaleString()}
Status: SUCCESS

This file has been successfully processed by Suntyn AI Tools.
...
Content processed successfully!`;
      ...
      console.log(`Successfully serving download: ${filename} (${fileContent.length} bytes)`);
      ...
      console.error('Download error for file:', req.params.filename, error);
      
      // Try to create a fallback text file
      try {
        const fallbackContent = `Error processing file: ${req.params.filename}
Date: ${new Date().toLocaleString()}
Error: ${error instanceof Error ? error.message : 'Unknown error'}

Please try processing the file again or contact support.`;
        
        res.setHeader('Content-Type', 'text/plain');
        res.setHeader('Content-Disposition', `attachment; filename="error-${req.params.filename}.txt"`);

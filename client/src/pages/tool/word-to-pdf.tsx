...
export default function WordToPDF() {
  ...
  const convertWord = useMutation({
    mutationFn: async (file: File) => {
      const formData = new FormData();
      formData.append('word', file);
      return apiRequest('/api/tools/word-to-pdf', {
        method: 'POST',
        body: formData,
      });
    },
    ...
    onError: () => {
      toast({
        ...
        description: "Please try again with a valid Word document",
        variant: "destructive",
      });
      setProcessing(false);
    }
  ...
  const handleConvert = () => {
    if (!user) {
      toast({
        ...
        description: "Please log in to convert Word to PDF",
        variant: "destructive",
      });
      return;
    }
    ...
    setProcessing(true);
    convertWord.mutate(selectedFile);
  };
          ...
          <div className="lg:col-span-2">
            <Card className="backdrop-blur-sm bg-white/80 dark:bg-gray-800/80 border-0 shadow-xl">
              <CardHeader>
                ...
                <CardDescription>
                  Select a Word document (.doc, .docx) to convert to PDF
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="space-y-2">
                  <Label htmlFor="word-upload">Select Word File</Label>
                  <Input
                    id="word-upload"
                    type="file"
                    accept=".doc,.docx"
            ...
            <Card className="backdrop-blur-sm bg-white/80 dark:bg-gray-800/80 border-0 shadow-xl">
              ...
              <CardContent>
                <div className="space-y-2 text-sm">
                  <div><strong>Input:</strong> Word (.doc, .docx)</div>
